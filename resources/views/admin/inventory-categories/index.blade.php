...
@section('title', 'Kategori Inventory')

@section('content')
<x-page-header title="Kategori Inventory" subtitle="Kategorikan barang wardrobe (Gaun, Aksesoris, Hijab, Sepatu, dll)">
    <x-slot:actions>
        <x-button href="{{ route('admin.inventory.index') }}" color="ghost"><i class="fas fa-shirt"></i> Inventory Wardrobe</x-button>
    </x-slot:actions>
</x-page-header>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div>
        <x-card title="Tambah Kategori" title-icon="fa-tags">
            <form action="{{ route('admin.inventory-categories.store') }}" method="POST" class="space-y-3">
                @csrf
                <x-input name="name" label="Nama Kategori" placeholder="cth: Gaun" required />
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 border-b border-gray-100 bg-cream/60">
                            ...
                            <th class="px-5 py-2.5 font-medium">Jumlah Barang</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 hover:bg-brand-50/30 transition-colors">
                            ...
                            <td class="px-5 py-3"><x-badge color="info">{{ $category->items_count }} barang</x-badge></td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.inventory-categories.destroy', $category) }}" onsubmit="return confirm('Hapus kategori ini?')">
                                        @csrf @method('DELETE')
...
<div id="edit-{{ $category->id }}" class="hidden fixed inset-0 z-[1100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        ...
        <form action="{{ route('admin.inventory-categories.update', $category) }}" method="POST" class="space-y-3">
            @csrf @method('PUT')
