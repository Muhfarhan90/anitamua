...
@section('content')
<x-page-header title="Kategori Vendor" subtitle="Kategorikan vendor (Photographer, Videographer, Decoration, MC, Catering, dll)">
    <x-slot:actions>
        <x-button href="{{ route('admin.vendors.index') }}" color="ghost"><i class="fas fa-store"></i> Master Vendor</x-button>
    </x-slot:actions>
</x-page-header>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div>
        <x-card title="Tambah Kategori" title-icon="fa-tags">
            <form action="{{ route('admin.vendor-categories.store') }}" method="POST" class="space-y-3">
                @csrf
                <x-input name="name" label="Nama Kategori" placeholder="cth: Photographer" required />
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 border-b border-gray-100 bg-cream/60">
                            ...
                            <th class="px-5 py-2.5 font-medium">Slug</th>
                            <th class="px-5 py-2.5 font-medium">Jumlah Vendor</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 hover:bg-brand-50/30 transition-colors">
                            ...
                            <td class="px-5 py-3"><x-badge color="info">{{ $category->vendors_count }} vendor</x-badge></td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.vendor-categories.destroy', $category) }}" onsubmit="return confirm('Hapus kategori ini?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="4"><x-empty-state icon="fa-tags" title="Belum ada kategori" /></td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
<div id="edit-{{ $category->id }}" class="hidden fixed inset-0 z-[1100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        ...
        <form action="{{ route('admin.vendor-categories.update', $category) }}" method="POST" class="space-y-3">
            @csrf @method('PUT')
